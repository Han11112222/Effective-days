...
def compute_weights_monthly(
    ...
    """같은 달에서 base_cat(평일_1) 중앙값 대비 비율로 월별 가중치 산정"""
    W = []
    for m in range(1,13):
        sub = df[df["월"]==m]
        if sub.empty:
...
def effective_days_by_month(df: pd.DataFrame, weights_monthly: pd.DataFrame) -> pd.DataFrame:
    ...
    def _note(r):
        note = []
        s, c = int(r.get("일수_명절_설날",0)), int(r.get("일수_공휴일_대체",0))
